...
const createNewBook = asyncHandler(async (req, res) => {
	const { title, author, genre, isbn, publishedYear, copiesAvailable } =
		req.body;
	...
	const duplicate = await Book.find({ title })
...
const updateBook = asyncHandler(async (req, res) => {
	const { id, title, author, isbn, genre, publishedYear, copiesAvailable } =
		req.body;
...
const deleteBook = asyncHandler(async (req, res) => {
	...
	if (borrowedBook) {
		return res
			.status(400)
			.json({
				message: "Book cannot be deleted as it has related borrow records",
			});
	}
